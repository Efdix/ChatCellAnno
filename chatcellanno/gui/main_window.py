...
class MainWindow(QMainWindow):
    ...
    def img_path_drag_enter(self, event):
        """处理图片路径框的拖入事件"""
        if event.mimeData().hasUrls():
            for url in event.mimeData().urls():
                if url.toLocalFile().lower().endswith((
                    '.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.gif', '.webp', '.xpm'
                )):
                    event.acceptProposedAction()
                    return
        event.ignore()

    def img_path_drop(self, event):
        """处理图片路径框的放下事件"""
        for url in event.mimeData().urls():
    ...
    def export_analysis_report(self):
        ...
        try:
            ...
            # 使用翻译系统中的模板 (Use template from translations)
            template = self.config.T("report_template")
            
            # 兼容旧版本格式化字段名或作为后备模板
            if not template or "{results}" not in template:
                template = "# ChatCellAnno Analysis Report\n\n## 1. Project Info\n- **Date**: {date}\n\n## 2. Cell Type Annotations\n{results}\n\n## 3. Enrichment Analysis\n{enrichment}\n\n---\n*Generated by ChatCellAnno*"

            # 统一字段名进行填充 (Ensure uniform keys for formatting)
